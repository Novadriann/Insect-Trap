...
// Modul RTC DS3231 (Dipindah ke pin U0TX dan U0RX)
// MENGORBANKAN Serial Monitor agar Flash LED bisa digunakan.
#define I2C_SDA 1
#define I2C_SCL 3
...
// Konfigurasi Kamera (Model AI-Thinker)
...
void setup() {
  // SERIAL MONITOR DIMATIKAN KARENA PIN 1 & 3 DIPAKAI OLEH RTC
  // Serial.begin(115200); 
  ...
  LoRaSerial.print(sensorData);
  delay(1000); 

  // ================= AMBIL & KIRIM GAMBAR DENGAN FLASH =================
  digitalWrite(FLASH_PIN, HIGH); // NYALAKAN LAMPU FLASH
  delay(1000); // Beri waktu 1 detik agar sensor beradaptasi
  
  camera_fb_t * fb = esp_camera_fb_get();
  
  digitalWrite(FLASH_PIN, LOW); // MATIKAN LAMPU FLASH segera setelah difoto
  ...
  if (fb) {
    ...
    for (size_t i = 0; i < fb->len; i += chunkSize) {
      ...
      LoRaSerial.write(fb->buf + i, len);
      delay(50); 
    }
    
    delay(100);
    LoRaSerial.print("---END---\n");
    esp_camera_fb_return(fb);
  }

  // ================= DEEP SLEEP =================
  uint64_t TIME_TO_SLEEP = 30; // Testing 30 detik
  esp_sleep_enable_timer_wakeup(TIME_TO_SLEEP * 1000000ULL);
  
  digitalWrite(FLASH_PIN, LOW); // Keamanan tambahan matikan flash sebelum tidur
  LoRaSerial.flush();
